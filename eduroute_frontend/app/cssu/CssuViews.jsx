...
export const ISSUReportsView = ({
  ...
  const filteredMovementRows = useMemo(() => {
    const rows = Array.isArray(reportData?.movementLogs) ? reportData.movementLogs : [];
    return rows.filter(row => {
      const location = String(row.locationLabel || '').toLowerCase();
      const status = String(row.movementStatus || '').toLowerCase();
      const employee = `${row.facultyName || ''} ${row.facultyId || ''}`.toLowerCase();
      const gateMatches = selectedGate === 'all' || location.includes(selectedGate === 'back_gate' ? 'back gate' : 'main gate');
      return gateMatches && (selectedStatus === 'all' || status === selectedStatus) && (!normalizedEmployeeSearch || employee.includes(normalizedEmployeeSearch));
    });
  }, [reportData, selectedGate, selectedStatus, normalizedEmployeeSearch]);
  const previewRows = useMemo(() => {
    return [...filteredMovementRows].sort((left, right) => {
      const leftTime = left?.occurredAt ? new Date(left.occurredAt).getTime() : 0;
      const rightTime = right?.occurredAt ? new Date(right.occurredAt).getTime() : 0;
      return logSortOrder === 'asc' ? leftTime - rightTime : rightTime - leftTime;
    }).slice(0, visibleRecordCount);
  }, [filteredMovementRows, visibleRecordCount, logSortOrder]);
  const hasMoreRecords = visibleRecordCount < filteredMovementRows.length;
    ...
    return <ISSUDesktopPage activeKey="reports" setView={setView} profileData={profileData} onLogout={onLogout} hideHeader>
      ...
      <div className="cssu-reports-hero-row">
        <div className="cssu-reports-hero-copy">
          <span className="cssu-desktop-kicker">Internal Logistics</span>
            <h1>History</h1>
          <p>Review recorded exit and return scan activity from ISSU gates.</p>
        </div>
      </div>
      ...
      <div className="cssu-reports-filter-row">
        ...
        <div className="cssu-reports-filter-field">
          ...
          <input className="cssu-reports-text-filter" value={employeeSearch} onChange={event => setEmployeeSearch(event.target.value)} placeholder="Search employee" aria-label="Search employee" />
        </div>

        <div className="cssu-reports-filter-field">
          <label>GATE</label>
          <div className="cssu-reports-select-shell"><select value={selectedGate} onChange={event => setSelectedGate(event.target.value)} aria-label="Gate"><option value="all">All Gates</option><option value="main_gate">Main Gate</option><option value="back_gate">Back Gate</option></select><ChevronDownIcon /></div>
        </div>

        <div className="cssu-reports-filter-field">
          <label>STATUS</label>
          <div className="cssu-reports-select-shell"><select value={selectedStatus} onChange={event => setSelectedStatus(event.target.value)} aria-label="Movement status"><option value="all">All Statuses</option><option value="verified">Verified</option><option value="flagged">Flagged</option></select><ChevronDownIcon /></div>
        </div>
      ...
      <div className="cssu-reports-grid">
        <section className="cssu-reports-preview-card">
          <div className="cssu-reports-preview-head">
            ...
            <div className="cssu-reports-preview-controls">
              ...
              <span className="cssu-reports-draft-pill">LIVE HISTORY</span>
            </div>
          </div>
        ...
        <aside className="cssu-reports-side-stack">
          <article className="cssu-reports-summary-card">
            <span>VISIBLE MOVEMENTS</span>
            <strong>{filteredMovementRows.length}</strong>
            <p>Exit and return scan records matching the selected filters.</p>

              <div className="cssu-reports-summary-metrics">
                <div><label>Exit Clearances</label><b>{reportData?.summary?.exitClearances ?? 0}</b></div>
                <div><label>Return Clearances</label><b>{filteredMovementRows.filter(row => String(row.eventLabel || '').toLowerCase() === 'return verification').length}</b></div>
          ...
          <article className="cssu-reports-banner-card">
            ...
            <h3>ISSU Security &amp; Movement Hub</h3>
          </article>
        </aside>
      </div>
    ...
    <div className="admin-dash-scroll cssu-scroll">
      ...
      <div className="cssu-mobile-reports-shell">
        <div className="cssu-mobile-reports-hero">
            <h2>History</h2>
          <p>Review recorded exit and return scan logs.</p>
        </div>
        ...
        <section className="cssu-mobile-reports-filter-card">
          ...
          <div className="cssu-mobile-reports-field">
            <label>Gate</label>
            <div className="cssu-mobile-reports-select"><select value={selectedGate} onChange={event => setSelectedGate(event.target.value)} aria-label="Gate"><option value="all">All Gates</option><option value="main_gate">Main Gate</option><option value="back_gate">Back Gate</option></select><ChevronDownIcon /></div>
          </div>

          <div className="cssu-mobile-reports-field">
            <label>Status</label>
            <div className="cssu-mobile-reports-select"><select value={selectedStatus} onChange={event => setSelectedStatus(event.target.value)} aria-label="Movement status"><option value="all">All Statuses</option><option value="verified">Verified</option><option value="flagged">Flagged</option></select><ChevronDownIcon /></div>
          </div>
    ...
    const loadAlerts = async () => {
      ...
      try {
        ...
        setAlerts(notificationRows.map(notification => ({
          ...
          actionLabelSecondary: notification.type === 'flagged' ? 'Open History' : 'Open Dashboard'
        })));
        setSummary({
